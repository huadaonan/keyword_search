...
if (!empty($keyword)) {
    ...
    $command = "cd " . escapeshellarg($base_dir) . " && LC_ALL=en_US.UTF-8 grep -inr --include='*.html' --include='*.htm' $escaped_keyword *  2>/dev/null";
    ...
    foreach ($output as $line) {
        ...
        if (count($parts) >= 3) {
            ...
            if (file_exists($full_path) && is_readable($full_path)) {
                // 构建访问URL，添加锚点参数和高亮参数
                $anchor = 'line_' . $line_number; // 使用行号作为锚点标识
                $file_url = $base_url . '/' . $file_path . '?highlight=' . urlencode($keyword) .  '#' . $anchor;
